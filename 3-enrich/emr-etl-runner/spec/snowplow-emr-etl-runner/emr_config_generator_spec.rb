...
describe EmrConfigGenerator do
  ...
  describe '#generate' do
    it { should respond_to(:generate).with(3).argument }

    it 'should write a proper emr cluster file' do
      filename = '/tmp/cluster.json'
      subject.generate(config, '1-0-0', filename)
  ...
  describe '#get_schema_name_from_version' do
    ...
    it 'should give back the proper schema name' do
      expect(subject.get_schema_name_from_version('1-0-0')).to eq(
        "com.snowplowanalytics.dataflowrunner/ClusterConfig/avro/1-0-0")
    end
  end
  ...
  describe '#get_schema' do
    ...
    it 'should give back the proper schema' do
      expect(subject.get_schema('1-0-0').length).to eq(5850)
    end
  end
  ...
  describe '#get_hbase_action' do
    it 'should give back a hbase bootstrap action' do
      expect(subject.send(:get_hbase_action, 'eu-west-1')).to eq({
        "name" => "Bootstrap action (installing HBase)",
        "scriptBootstrapAction" => {
          "path" => "s3://eu-west-1.elasticmapreduce/bootstrap-actions/setup-hbase",
          "args" => []
        }
      })
    end
  ...
  describe '#get_lingual_action' do
    it 'should give back a lingual bootstrap action' do
      expect(subject.send(:get_lingual_action, '1.8.0')).to eq({
        "name" => "Bootstrap action (installing Lingual)",
        "scriptBootstrapAction" => {
          "path" => "s3://files.concurrentinc.com/lingual/1.8.0/lingual-client/install-lingual-client.sh",
          "args" => []
        }
      })
    end
  ...
  describe '#get_bootstrap_actions' do
    it 'should build a list of actions starting with no actions (thrift + legacy)' do
      expect(subject.send(:get_bootstrap_actions,
        [], 'thrift', true, 'eu-west-1', '1.8.0', nil, nil)).to eq([
          {
    ...
    it 'should incorporate the specified list of actions' do
      ...
      ], 'not-thrift', false, 'eu-west-1', '1.9.0', nil, nil)).to eq([
        { "name" => "something", "scriptBootstrapAction" => { "path" => "path", "args" => [] } },
        { "name" => "another thing", "scriptBootstrapAction" => { "path" => "another path", "args" => [] } },
        {
          "name" => "Bootstrap action (ami bootstrap script)",
    ...
    it 'should incorporate hbase and lingual actions' do
      expect(subject.send(:get_bootstrap_actions,
        [], 'not-thrift', false, 'eu-west-1', '1.9.0', '2.0.0', '3.8.1')).to eq([
        {
          "name" => "Bootstrap action (ami bootstrap script)",
          "scriptBootstrapAction" => {
            "path" => "s3://snowplow-hosted-assets/common/emr/snowplow-ami4-bootstrap-0.2.0.sh",
        ...
        },
        {
          "name" => "Bootstrap action (installing Lingual)",
          "scriptBootstrapAction" => {
            "path" => "s3://files.concurrentinc.com/lingual/3.8.1/lingual-client/install-lingual-client.sh",
            "args" => []
          }
        }
      ])
